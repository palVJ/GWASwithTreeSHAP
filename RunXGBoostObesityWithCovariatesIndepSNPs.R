...
library(devtools)
document()
setwd("/SAY/dbgapstg/dbgap/work/paalvj/XGBoost/XGBoostOnSNPData")
document()
...
#Include covariates data:
library(data.table)
CovData = fread("/SAY/dbgapstg/dbgap/work/paalvj/ObesityCovOnlyCaucasiansIndepQC.txt",header = TRUE, sep  = "\t", colClasses = c("IID" = "character"))
#Which IIDs to include.
IIDsToInclude = fread(paste("/SAY/dbgapstg/dbgap/work/paalvj/RawFiles/IIDsToIncludeIndepSNPs",shID,".txt",sep = ""),header = TRUE, sep = "\t",
select = c("IID"),colClasses = c("IID"="character"))
CovData = subset(CovData,CovData$IID %in% as.character(IIDsToInclude$IID))

RUN = XGBoostOnSNPdata_cv(TrainData = c(paste("/SAY/dbgapstg/dbgap/work/paalvj/RawFiles/SNP_add_","SubsetIndepSNPs",shID,"_Trai1.raw",sep = ""),
...
fwrite(RUN$Importance,paste("/SAY/dbgapstg/dbgap/work/paalvj/ImportanceTables/BMI_ImportanceUnifWholeGenomeWithCovsIndepSNPs","Subset",shID,"TestData",WhichIsTestdata,".txt",
sep = ""),quote = FALSE, row.names = FALSE, sep = "\t",nThread=2)


#Save R-object models:
save(RUN, file = paste("/SAY/dbgapstg/dbgap/work/paalvj/XGBmodels/XGB_UnifWholeGenomeWithCovsIndepSNPs","Subset",shID,"Testdata",WhichIsTestdata,".RData",sep = ""))
